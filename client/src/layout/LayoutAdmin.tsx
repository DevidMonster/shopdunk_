...
const LayoutAdmin = () => {
  ...
  const ButtonTrigger = (
    <button className="bg-greenPrimary text-white w-full font-semibold">
      {collapsed ? "Hiện" : "Ẩn"}
    </button>
  );
  const {
    ...
    <Layout style={{ minHeight: "100vh" }}>
      ...
      {open ? (
        <div
          ...
          className="fixed top-0 right-0 z-[150] w-screen h-full bg-[rgba(0,0,0,0.1)] md:hidden md:opacity-0 md:invisible"
        ></div>
      ) : (
        ""
      )}
